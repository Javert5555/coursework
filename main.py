...
def join_arrays_to_string(arrays):
    ...
    arrays = [[str(num) for num in subarray] for subarray in arrays]
    
    # Объединение массивов в строку
    result = ' '.join([' '.join(subarray) for subarray in arrays])
...
def get_solution(init_text, k_val):
    ...
    for initial_code_text in new_initial_text:
        ...
        print('остаток', get_pols_div(code_word, gen_pol, m))
        ...
        # print(p3)
        # print(len(gf_table))
    # print(new_initial_text_code_words)
    return {
        'initial_text': initial_text,
