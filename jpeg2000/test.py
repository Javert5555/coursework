...
# def perform_dc_level_shift(image):
#     h, w = image.shape[:2]
#     channel_count = image.shape[2]

#     for channel in range(channel_count):
#         channel_image = image[:, :, channel]
#         shift_matrix = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
#         for i in range(h - 4):
#             for j in range(w - 4):
#                 pixel = channel_image[i, j]
#                 for k in range(5):
#                     if i + k < h and j + k < w:
#                         new_pixel = channel_image[i + k, j + k]
#                         channel_image[i, j] = pixel
#                         pixel = new_pixel
#                     elif i + k < h:
#                         new_pixel = channel_image[i + k, j]
#                         channel_image[i, j] = pixel
#                         pixel = new_pixel
#                     elif j + k < w:
#                         new_pixel = channel_image[i, j + k]
#                         channel_image[i, j] = pixel
#                         pixel = new_pixel
#                 shift_matrix = np.roll(shift_matrix, k, axis=0)

#     return image

# image = cv2.imread('./assets/700x1000.jpg')
# shifted_image = perform_dc_level_shift(image)
...
class JPEG2000(object):
    ...
    def __init__(self, file_path="./assets/img_brightness_shift_arr.jpg", quant=True, lossy=True, debug=False, tile_size=2**10):
        """
        JPEG2000 algorithm
        Initial parameters:
