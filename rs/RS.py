...
class RS:
    @staticmethod
    def get_gf_table():
        return [1, 2, 4, 8, 16, 32, 64, 128, 29, 58, 116, 232, 205, 135, 19, 38, 76, 152, 45, 90, 180, 117, 234,
    ...
    def get_mult(num1, num2):
        index1 = RS.get_gf_table().index(num1)
        index2 = RS.get_gf_table().index(num2)
        final_index = (index1 + index2) % (len(RS.get_gf_table()) - 1)
        return RS.get_gf_table()[final_index]
    ...
    def get_pols_div(dividend, divisor, m):
        ...
        divisor_indeces = list(reversed([RS.get_gf_table().index(i) for i in divisor]))
        ...
        for i in range(len(dividend)):
            ...
            if dividend_values[i] != 0:
                degree_difference = RS.get_gf_table().index(dividend_values[i]) - temp_divisor_indeces[0]
                # print(degree_difference)
                temp_divisor_indeces = [(i + degree_difference) % (len(RS.get_gf_table()) - 1) for i in temp_divisor_indeces]
                ...
                for j in range(i, i + len(divisor_indeces)):
                    ...
                    if(dividend_values[j] != 0):
                        first = RS.get_gf_table()[RS.get_gf_table().index(dividend_values[j])]
                    dividend_values[j] = first ^ RS.get_gf_table()[temp_divisor_indeces[temp_index]]
                    # if(dividend_values[j] != 0):
                        # dividend_values[j] = get_gf_table().index(dividend_values[j])
    ...
    def get_pols_mult(pol1, pol2):
        ...
        pol1_indeces = [RS.get_gf_table().index(i) for i in pol1]
        # print(pol2)
        pol2_indeces = [RS.get_gf_table().index(i) for i in pol2]
        ...
        for i in range(len(pol1_indeces)):
            for j in range(len(pol2_indeces)):
                ...
                coefficients_values[i+j].append(RS.get_gf_table()[(pol1_indeces[i] + pol2_indeces[j]) % (len(RS.get_gf_table()) - 1)])
    ...
    def get_gen_pol(m):
        if (m == 0):
            return [1]
        pols = [[RS.get_gf_table()[i], 1] for i in range(1, m + 1)]
        temp_pol = pols[0]
        for i in range(1, m):
            temp_pol = RS.get_pols_mult(temp_pol, pols[i])
        return temp_pol
    ...
    def split_text_into_arrays(text, k):
        ...
        decimal_representation = RS.get_init_text_split(text)
    ...
    def encode(init_text, k_val):
        ...
        n = len(RS.get_gf_table()) - 1
        # k = 100
        k = k_val
        m = n - k
            
        gen_pol = RS.get_gen_pol(m)
        # print(gen_pol)
        ...
        new_initial_text = RS.split_text_into_arrays(initial_text, k)
        # print('new_initial_text', new_initial_text)
        new_initial_text_code_words = []
        for initial_code_text in new_initial_text:
            remainder_div = RS.get_pols_div(initial_code_text, gen_pol, m)
